...
{{-- Formulaire de modification d'un emprunteur --}}

@extends('app.master')

@section('title', 'Modifier Emprunteur - Libra')
    ...
    <div class="relative z-10 flex items-center gap-6">
        ...
        <div>
            <h1 class="text-4xl font-bold text-white mb-2 tracking-tight">
                <i class="fas fa-user-edit mr-3 text-teal-400"></i>Modifier l'Emprunteur
            </h1>
            ...
            <p class="text-indigo-300 text-sm mt-1">
                <i class="fas fa-info-circle mr-2"></i>Modification de : <span class="font-semibold">{{ $borrower->name }}</span>
            </p>
        </div>
    </div>
</div>
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
        ...
        <div class="px-8 py-8">
            <form method="POST" action="{{ route('borrowers.update', $borrower->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $borrower->name) }}" required 
                ...
                <div>
                    ...
                    <input type="text" name="matricule" value="{{ old('matricule', $borrower->matricule) }}" required 
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $borrower->email) }}" required 
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-rose-500 focus:ring-4 focus:ring-rose-100">
                        ...
                        <option value="student" {{ old('type', $borrower->type)=='student'?'selected':'' }}>🎓 Étudiant</option>
                        <option value="teacher" {{ old('type', $borrower->type)=='teacher'?'selected':'' }}>👨‍🏫 Enseignant</option>
                        <option value="staff" {{ old('type', $borrower->type)=='staff'?'selected':'' }}>👔 Personnel</option>
                    </select>
